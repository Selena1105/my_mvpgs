...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, test_cam_names_list, load_fg_mask=False, dtu_mask_path=None):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, K=K, bounds=bds, fg_mask=dtu_mask, FovY=FovY, FovX=FovX, image=image,
                              ...
                              is_test=image_name.split(".")[0] in test_cam_names_list)
...
def readColmapSceneInfo(path, images, eval, llffhold=8, dataset='LLFF', input_n=3, mvs_config_path=None, use_colmap_init=False, pc_downsample=0.1, stage='train', dtu_mask_path=None):
    ...
    if eval:
        with open(os.path.join(path, "test.txt"), 'r', encoding='utf-8') as file:
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), 
                                           load_fg_mask=load_fg_mask, dtu_mask_path=dtu_mask_path, test_cam_names_list=test_cam_names_list)
    ...
    if eval:
        ...
        elif dataset == 'mmor' or '4dor' or 'lerf_ovs':
